...
export default function WhatsAppButton() {
  ...
  return (
    ...
    >
      {/* The WhatsApp glyph is two sub-paths: outer speech-bubble + inner
          handset cutout. SVG's default `nonzero` fill-rule fills both
          solid (no hole), which renders as a green blob instead of the
          recognizable icon. `evenodd` makes the inner sub-path subtract
          from the outer, giving the right shape. */}
      <svg
        viewBox="0 0 24 24"
        className="h-7 w-7"
        fill="currentColor"
